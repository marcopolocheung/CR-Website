...
export function restaurantFromUrl(url) {
    const raw = url.searchParams.get('restaurant');
    if (raw === null || raw === '')
        return DEFAULT_RESTAURANT;
    if (!RESTAURANT_RE.test(raw) || !isValidRestaurant(raw))
        return null;
    return raw;
}
...
const MAX_SLOTS = 200;
export function goldenKey(weekStart, restaurant = DEFAULT_RESTAURANT) {
    return `r:${restaurant}:golden:${weekStart}`;
}
export function legacyGoldenKey(weekStart) {
    return `golden:${weekStart}`;
}
export function goldenMonthRevKey(monthKey, restaurant = DEFAULT_RESTAURANT) {
    return `r:${restaurant}:golden:month-rev:${monthKey}`;
}
export function legacyGoldenMonthRevKey(monthKey) {
    return `golden:month-rev:${monthKey}`;
}
function shiftGoldenDate(isoDate, days) {
...
export function goldenMonthsForWeek(weekStart) {
    if (!WEEK_RE.test(weekStart))
        return [];
    return [...new Set([weekStart.slice(0, 7), shiftGoldenDate(weekStart, 6).slice(0, 7)])];
}
export async function readGoldenMonthRev(env, monthKey, restaurant = DEFAULT_RESTAURANT) {
    try {
...
async function bumpGoldenMonthRevs(env, weekStart, restaurant = DEFAULT_RESTAURANT) {
...
async function readGoldenDoc(env, restaurant, weekStart) {
...
function isValidStoredTimeRange(value) {
...
function isValidStoredEmployee(value) {
    if (!value || typeof value !== 'object')
        return false;
    ...
    if (!employee.recurringAvailability || typeof employee.recurringAvailability !== 'object')
        return false;
    const availability = employee.recurringAvailability;
    for (const [day, ranges] of Object.entries(availability)) {
        if (!ROSTER_DAYS.includes(day))
            return false;
        if (!Array.isArray(ranges) || ranges.length > MAX_RANGES_PER_DAY || !ranges.every(isValidStoredTimeRange))
            return false;
    }
    ...
    if (typeof employee.active !== 'boolean')
        return false;
    if (employee.newHire !== undefined && typeof employee.newHire !== 'boolean')
        return false;
    return true;
}
export function validateRosterBody(body) {
    if (!body || typeof body !== 'object')
        return null;
    const { employees, baseRev } = body;
    if (typeof baseRev !== 'number' || !Number.isInteger(baseRev) || baseRev < 0)
        return null;
    if (!Array.isArray(employees) || employees.length > MAX_EMPLOYEES)
        return null;
    if (!employees.every(isValidStoredEmployee))
        return null;
    const ids = new Set(employees.map((employee) => employee.id));
    if (ids.size !== employees.length)
        return null;
    return {
        employees: employees.map((employee) => ({ ...employee, newHire: employee.newHire ?? false })),
        baseRev,
    };
}
export function normalizeRosterDoc(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const doc = raw;
    if (doc.v !== 1)
...
function isValidStoredTemplateSlot(value) {
    if (!value || typeof value !== 'object')
        return false;
    const slot = value;
    ...
    if (typeof slot.label !== 'string' || slot.label.length === 0 || slot.label.length > MAX_LABEL_CHARS)
        return false;
    if (typeof slot.start !== 'number' || !Number.isInteger(slot.start) || slot.start < 0 || slot.start >= MAX_MINUTES_PER_DAY)
        return false;
    if (typeof slot.end !== 'number' || !Number.isInteger(slot.end) || slot.end <= slot.start || slot.end > MAX_MINUTES_PER_DAY)
        return false;
...
export function validateTemplateBody(body) {
    if (!body || typeof body !== 'object')
        return null;
    const { template, baseRev } = body;
    if (typeof baseRev !== 'number' || !Number.isInteger(baseRev) || baseRev < 0)
        return null;
    if (!template || typeof template !== 'object')
        return null;
    const candidate = template;
    if (Object.keys(candidate).some((key) => !ROSTER_DAYS.includes(key)))
        return null;
    const result = {};
    for (const day of ROSTER_DAYS) {
        const slots = candidate[day] ?? [];
        if (!Array.isArray(slots) || slots.length > MAX_SLOTS_PER_DAY || !slots.every(isValidStoredTemplateSlot))
            return null;
        result[day] = slots;
    }
    return { template: result, baseRev };
}
export function normalizeTemplateDoc(raw) {
    if (!raw || typeof raw !== 'object')
        return null;
    const doc = raw;
    if (doc.v !== 1)
